...
export default function SuggestPlaceModal({ isOpen, onClose }) {
  ...
  const [filterOptions, setFilterOptions] = useState({ directions: [], seasons: [], objectTypes: [], accessibility: [] })
  ...
  const hasCategories = ['directions', 'seasons', 'objectTypes', 'accessibility'].some((k) => filterOptions[k].length > 0)
        ...
        <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
          <div className={styles.modalHeader}>
            <div className={styles.modalTitleBlock}>
              <h2 className={styles.modalTitle}>Предложить место</h2>
              {!success && !(!user) && (
                <p className={styles.modalSubtitle}>Заполните форму — мы рассмотрим и опубликуем на платформе</p>
              )}
            </div>
            ...
            {!user ? (
              ...
              <form onSubmit={handleSubmit} style={{ display: 'contents' }}>
                <div className={styles.card}>
                  ...
                  <div className={styles.field}>
                    ...
                    <input type="text" className={styles.input} value={formData.title}
                      onChange={(e) => handleChange('title', e.target.value)} placeholder="Название места" />
                  </div>
                  ...
                  <div className={styles.field}>
                    ...
                    <RichTextEditor value={formData.shortDescription}
                      ...
                      placeholder="Короткое описание для карточки..." minHeight={90} />
                  </div>
                  ...
                  <div className={styles.field}>
                    ...
                    <RichTextEditor value={formData.description}
                      ...
                      placeholder="Подробное описание места..." minHeight={140} />
                  </div>

                  <div className={styles.twoCol}>
                    <div className={styles.field}>
                      <label className={styles.label}>Как добраться</label>
                      <RichTextEditor value={formData.howToGet}
                        onChange={(v) => handleChange('howToGet', v)}
                        placeholder="Описание маршрута..." minHeight={90} />
                    </div>
                    <div className={styles.field}>
                      <label className={styles.label}>Важно знать</label>
                      <RichTextEditor value={formData.importantInfo}
                        onChange={(v) => handleChange('importantInfo', v)}
                        placeholder="Важная информация для посетителей..." minHeight={90} />
                    </div>
                  </div>
                </div>

                <div className={styles.card}>
                  <h3 className={styles.cardTitle}>Местоположение на карте</h3>
                  <div className={styles.mapModeRow}>
                    <button type="button"
                      className={`${styles.mapModeBtn} ${mapSearchMode === 'byName' ? styles.mapModeBtnActive : ''}`}
                      onClick={() => setMapSearchMode('byName')}>По названию</button>
                    <button type="button"
                      className={`${styles.mapModeBtn} ${mapSearchMode === 'byCoordinates' ? styles.mapModeBtnActive : ''}`}
                      onClick={() => setMapSearchMode('byCoordinates')}>По координатам</button>
                  </div>
                  <YandexMapPicker lat={formData.latitude} lng={formData.longitude} searchMode={mapSearchMode}
                    ...
                    <div className={styles.gallery}>
                      {allGalleryItems.map((item, index) => (
                        <div key={index} className={styles.galleryItem}>
                          <img src={item.type === 'saved' ? getImageUrl(item.url) : item.preview} alt="" className={styles.galleryImg} />
                          <div className={styles.galleryControls}>
                            <button type="button" onClick={() => moveGalleryItem(index, -1)} disabled={index === 0}><ChevronLeft size={12} /></button>
                            <button type="button" onClick={() => removeGalleryItem(index)}><X size={12} /></button>
                            <button type="button" onClick={() => moveGalleryItem(index, 1)} disabled={index === allGalleryItems.length - 1}><ChevronRight size={12} /></button>
                          </div>
                        </div>
                      ))}
                      <button type="button" className={styles.addGalleryBtn} onClick={() => galleryInputRef.current?.click()}>
                        <Upload size={16} /><span>Добавить</span>
                      </button>
                ...
                <div className={styles.card}>
                  ...
                  <div className={styles.twoCol}>
                    <div className={styles.field}>
                      <label className={styles.label}>Ссылка на карту</label>
                      <input type="text" className={styles.input} value={formData.mapUrl}
                        onChange={(e) => handleChange('mapUrl', e.target.value)} placeholder="https://..." />
                    </div>
                    <div className={styles.field}>
                      <label className={styles.label}>Аудиогид (ссылка)</label>
                      <input type="text" className={styles.input} value={formData.audioGuide}
                        onChange={(e) => handleChange('audioGuide', e.target.value)} placeholder="https://..." />
                    </div>
                  </div>
                  <div className={styles.field}>
                    <label className={styles.label}>Видео (VK Video / YouTube)</label>
                    <input type="text" className={styles.input} value={formData.video}
                      onChange={(e) => handleChange('video', e.target.value)} placeholder="https://vkvideo.ru/..." />
                  </div>
                </div>
