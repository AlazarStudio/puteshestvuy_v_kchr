/**
 * Транслитерация русского текста в латиницу для ЧПУ
 * @param {string} text - Текст для транслитерации
 * @returns {string} - Транслитерированный текст в нижнем регистре с дефисами
 */
export function transliterate(text) {
  if (!text) return ''

  const transliterationMap = {
    'а': 'a', 'б': 'b', 'в': 'v', 'г': 'g', 'д': 'd', 'е': 'e', 'ё': 'yo',
    'ж': 'zh', 'з': 'z', 'и': 'i', 'й': 'y', 'к': 'k', 'л': 'l', 'м': 'm',
    'н': 'n', 'о': 'o', 'п': 'p', 'р': 'r', 'с': 's', 'т': 't', 'у': 'u',
    'ф': 'f', 'х': 'h', 'ц': 'ts', 'ч': 'ch', 'ш': 'sh', 'щ': 'sch',
    'ъ': '', 'ы': 'y', 'ь': '', 'э': 'e', 'ю': 'yu', 'я': 'ya',
    'А': 'A', 'Б': 'B', 'В': 'V', 'Г': 'G', 'Д': 'D', 'Е': 'E', 'Ё': 'Yo',
    'Ж': 'Zh', 'З': 'Z', 'И': 'I', 'Й': 'Y', 'К': 'K', 'Л': 'L', 'М': 'M',
    'Н': 'N', 'О': 'O', 'П': 'P', 'Р': 'R', 'С': 'S', 'Т': 'T', 'У': 'U',
    'Ф': 'F', 'Х': 'H', 'Ц': 'Ts', 'Ч': 'Ch', 'Ш': 'Sh', 'Щ': 'Sch',
    'Ъ': '', 'Ы': 'Y', 'Ь': '', 'Э': 'E', 'Ю': 'Yu', 'Я': 'Ya'
  }

  // Транслитерируем текст
  let result = text
    .split('')
    .map(char => transliterationMap[char] || char)
    .join('')

  // Преобразуем в нижний регистр
  result = result.toLowerCase()

  // Заменяем пробелы и специальные символы на дефисы
  result = result
    .replace(/[^\w\s-]/g, '') // Удаляем все кроме букв, цифр, пробелов и дефисов
    .replace(/\s+/g, '-')      // Заменяем пробелы на дефисы
    .replace(/-+/g, '-')       // Убираем множественные дефисы
    .replace(/^-|-$/g, '')    // Убираем дефисы в начале и конце

  return result
}

/**
 * Генерирует slug из названия маршрута
 * @param {string} title - Название маршрута
 * @returns {string} - Slug для URL
 */
export function generateSlug(title) {
  return transliterate(title)
}
